...
public class MainActivity extends AppCompatActivity implements StatusDialogFragment.Observer, MainPresenter.MainView {
    ...
    // todo make logout user apart of view
    @Override
    ...
    @Override
    public void enableFollowButton(boolean status) {
        followButton.setEnabled(status);
    }
    ...
    @Override
    public void showPostingToast(boolean toastStatus) {
        if (toastStatus) {
            postingToast.show();
        } else {
            postingToast.cancel();
